...
subj=$(ls -d /import/monstrum/fndm2_new/subjects/*/) #rk edited
...
outfile=level2_fe_20150514//mask.nii.gz  #change to 2nd level FSF name
featname=level1_stats_20130903.feat  #change to your actual 1st level feat name
#cleanup logs
rm -f $joblist
...
for s in $subj; do
	...
	#affinenum=$(ls $s/*{card,face}*/coregistration/ep2struct.mat | wc  | awk '{print $1}')
 	affinenum=$(ls $s/*itc*/coregistration/ep2struct.mat | wc  | awk '{print $1}') #rk edited
	 if [ "$affinenum" != 4 ]; then
	...
	def=$(ls $s/*_mprage/${scanid}_brain_micobc_to_mni2mm_warp.nii.gz 2> /dev/null)  #you willneed to change these
        struct=$(ls $s/*_mprage/${scanid}_t1_brain_micobc.nii.gz 2> /dev/null)  #change also
...
#	echo $s >> $joblist  #comment out for non-grid testing
	...
	#/import/monstrum/fndm2_new/progs/ITC/dramms_reg_apply_and_level2_fe_20130905.sh $s  #check path to make sure this is right
 	/import/monstrum/fndm2_new/progs/ITC/dramms_reg_apply_and_level2_fe_20150514.sh $s  #rk edited
done
...
qsub -V -q veryshort.q -S /bin/bash -o ~/sge_out -e ~/sge_out -t 1-${ntasks} /import/monstrum/fndm2_new/progs/ITC/dramms_reg_apply_and_level2_fe_20150514.sh $joblist
