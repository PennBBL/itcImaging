...
#Multi-Subject FNDM2
runs=$(ls -d /import/monstrum/fndm2_new/subjects/*/behavioral/RTG*.txt)

runlist=/import/monstrum/fndm2_new/progs/behavioral/fndm_combine_running.txt
...
for r in $runs; do
	...
	subj=$(echo $r | cut -d/ -f8 | cut -d- -f2)
	series=$(echo $r | cut -d/ -f8 | cut -d- -f3 | cut -d. -f1)
	...
	cat ${subjdir}/${subjid}/behavioral/RTG${series}_1ITCscanner1LLA-${subj}-${series}.txt >> ${subjdir}/${subjid}/behavioral/ITC_combined_${subj}--.txt
