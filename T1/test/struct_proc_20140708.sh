...
#example: bet 6843_t1.nii.gz 6843_t1_brain.nii.gz

t1brain=$(ls ${scanid}_t1_brain.nii.gz)
#would add if statement to see if present or not as above, etc etc


#segment T1 image if not already done
...
if [ ! -e "$mico_out" ]; then
	echo "running MICO segementation"
	...
	mico_out=$(ls ${scanid}_brain_micoseg.nii.gz 2> /dev/null)
	if [ ! -e "$mico_out" ]; then
...
if [ ! -e "$dramms_out" ]; then
	...
	t1brain_mico=$(ls ${scanid}_brain_micobc.nii.gz 2> /dev/null)
