...
for s in $subj; do
	...
	t1brain=$(ls ${mpragedir}/${scanid}_t1_brain.nii.gz)
	if [ ! -e "$t1brain" ]; then
...
qsub -V -q all.q -S /bin/bash -o ~/sge_out -e ~/sge_out -t 1-${ntasks} /import/monstrum/fndm2_new/progs/T1/struct_proc_V6.sh $joblist 
