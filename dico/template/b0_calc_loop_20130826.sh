subjects=$(ls -d /import/monstrum/day2_fndm/subjects/*7800)
...
for s in $subjects; do
	...
	t1head=$(ls $s/*mprage*/${scanid}_n3.nii.gz 2> /dev/null)
	t1brain=$(ls $s/*mprage*/${scanid}_n3_maskstr.nii.gz 2> /dev/null)
	...
	echo ""
	echo "running b0calc v2"
	echo $s >> ${logdir}/b0_calc_loop_secondrun.txt

	/import/monstrum/BBL_scripts/melliott/dico_b0calc_v2.sh -xmFS2 -T $t1head -B $t1brain $s/b0map/${scanid} $b0dicoms  #note mark reccomends the x/m flags
